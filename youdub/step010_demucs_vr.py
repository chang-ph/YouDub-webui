...
def separate_audio(
    ...
    """
    Input: <folder>/audio.wav
    Output: <folder>/audio_vocals.wav, <folder>/audio_instruments.wav
    """
...
def separate_audio2(
    ...
    logs = client.containers.run(
        "spleeter:latest",
        ["separate", "-p", "spleeter:2stems", "-o", folder, "audio.wav"],
        auto_remove=True,
        network_mode="host",
        volumes={
            folder: {"bind": folder, "mode": "rw"},
            # f"{pathlib.Path.home()}/.julia": {'bind': f'{pathlib.Path.home()}/.julia', 'mode': 'rw'},
        },
    )
    logger.info(f"Spleeter logs: {logs}")
    client.close()

    vocal_output_path = os.path.join(folder, "audio_vocals.wav")
    instruments_output_path = os.path.join(folder, "audio_instruments.wav")
    shutil.copy(Path(folder) / "audio" / "vocals.wav", vocal_output_path)
    logger.info(f"Vocals saved to {vocal_output_path}")

    shutil.copy(Path(folder) / "audio" / "accompaniment.wav", instruments_output_path)
    logger.info(f"Instruments saved to {instruments_output_path}")
...
def separate_all_audio_under_folder(
    ...
    for subdir, dirs, files in os.walk(root_folder):
        ...
        if "audio_vocals.wav" not in files:
            separate_audio2(subdir, model_name, device, progress, shifts)
